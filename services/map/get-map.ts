...
const getAddressType = (osmType: string, osmClass: string): 'building' | 'street' | 'district' => {
...
const getFeatureType = (placeTypes: string[]): 'building' | 'street' | 'district' => {
  if (placeTypes.includes('poi') || placeTypes.includes('address')) return 'building';
  if (placeTypes.includes('street') || placeTypes.includes('road')) return 'street';
  return 'district';
};
  ...
  async (query: string): Promise<Address[]> => {
    try {
      ...
      return data.map((item: any, index: number) => ({
        ...
        nameKh: item.name && item.name !== item.display_name.split(',')[0] ? item.name : undefined,
  ...
  async (query: string): Promise<Address[]> => {
    try {
      ...
      return data.features?.map((feature: any, index: number) => ({
        ...
        ].filter(Boolean).join(', ') || feature.properties.osm_value,
        coordinates: {
          lat: feature.geometry.coordinates[1],
          lng: feature.geometry.coordinates[0]
        },
        type: getAddressType(feature.properties.osm_value, feature.properties.osm_key),
        source: 'Photon'
      })) || [];
    } catch (error) {
...
export async function searchAddresses(query: string, mapboxApiKey?: string): Promise<Address[]> {
  ...
  try {
    ...
    allResults.forEach(address => {
      const isDuplicate = uniqueResults.some(existing => 
        Math.abs(existing.coordinates.lat - address.coordinates.lat) < threshold &&
        Math.abs(existing.coordinates.lng - address.coordinates.lng) < threshold
      );
